...
class Generate6x4 {
    List<String> unique6marks = new ArrayList<String>();
    List<String> unique4marks = new ArrayList<String>();
    List<String> marks6 = new ArrayList<String>();
    List<String> marks4 = new ArrayList<String>();
    ...
    void generateQuestionPaper() throws Exception {
        ...
        for (int i = 1; i <= sourceTotal; i++) {
            ...
            if (marksCheck == 6) {
                marks6.add(row.getCell(1).getStringCellValue());
            } else if (marksCheck == 4) {
                marks4.add(row.getCell(1).getStringCellValue());
            } else {
                continue;
            }
        }
        for (int i = 1; i <= 4; i++) {
            int random = ThreadLocalRandom.current().nextInt(1, marks6.size());
            String question1 = marks6.get(random - 1);
            if (!unique6marks.contains(question1)) {
                unique6marks.add(question1);
            } else {
                i--;
            }
            random = ThreadLocalRandom.current().nextInt(1, marks4.size());
            String question2 = marks4.get(random - 1);
            if (!unique4marks.contains(question2)) {
                unique4marks.add(question2);
            } else {
                i--;
            }
        }
        ...
        for (int i = 1; i <= 4; i++) {
            ...
            if (i % 2 == 0) {
                run2.setText(unique4marks.get(i - 1));
            } else {
                run2.setText(unique6marks.get(i - 1));
            }
